...
function createWindowSurface(s) {
  ...
  if (mw && !mw.isDestroyed()) {
    s.win = mw
    if (!s._listenersWired) {
      s._listenersWired = true
...
function registerBrowserControl(first, second) {
  const getMainWindow = typeof first === 'function' ? first : (typeof second === 'function' ? second : () => null)
  ...
  ipcMain.on('browser:tab-action', (e, { action, tabId, arg } = {}) => {
    ...
    switch (action) {
      ...
      case 'toggle-reader': {
        if (t) toggleReaderMode(t.view.webContents)
        break
      }
      case 'screenshot': {
        if (t) {
          t.view.webContents.capturePage().then((img) => {
            clipboard.writeImage(img)
            const dlPath = path.join(app.getPath('downloads'), `Yogatik_Screenshot_${Date.now()}.png`)
            fs.writeFile(dlPath, img.toPNG(), () => {})
            if (s.win && !s.win.isDestroyed()) {
              s.win.webContents.executeJavaScript(`alert('Screenshot copied to clipboard and saved to Downloads!')`).catch(() => {})
            }
          }).catch(() => {})
        }
        break
