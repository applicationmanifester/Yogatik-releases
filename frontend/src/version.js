...
export const APP_RELEASES = [
  {
    ...
    "highlights": [
      ...
      "Tool Call Leakage Shield: Enhanced syntax stripper and recovery filters prevent raw JSON tool blocks ({\"tool_calls\": [...]}) from leaking into user-facing prose bubbles",
      "Path Hallucination Reflection: Automatically detects when a model hallucinates the tool name as a file path (e.g. path: 'fs_read') and provides immediate self-correction guidance",
      "Relentless Mode Action Uncapping: Turns running with /relentless, /loop, or relentless mode operate with unbounded round ceilings (Infinity) to ensure multi-step engineering tasks finish completely"
    ],
    "sections": [
      {
        "category": "🤖 Autonomous Agent & Loop Defense",
        "items": [
          {
            "title": "Repetitive Failure Circuit Breaker",
            "description": "Monitors consecutive failures per tool. When a tool fails 2+ times consecutively, the engine injects a priority circuit-breaker directive commanding the model to stop looping and discover real paths using fs_find_files or fs_list."
          },
          {
            "title": "Tool Name Path Hallucination Guard",
            "description": "toolReflection now checks if the target path equals the tool name (e.g. fs_read), explaining the mistake to the model instantly before rounds are burned."
          },
          {
            "title": "Relentless Depth Uncapping",
            "description": "Removes the default 25-round ceiling when relentlessMode is active, allowing deep exploratory workflows to run until resolution."
          }
        ]
      },
      {
        "category": "⚡ Windows Path Precision & UI Leak Protection",
        "items": [
          {
            "title": "Windows Path Normalization in repairJson",
            "description": "Normalizes Windows drive paths like C:\\Users\\... inside JSON tool arguments with a robust function replacer, ensuring reliable parsing."
          },
          {
            "title": "Zero-Leak Prose Cleansing",
            "description": "stripToolCallSyntax now cleans bare and fenced JSON tool arrays so raw tool calls are never dumped into chat bubbles when budgets run out."
          }
        ]
      }
    ]
  ...
  {
    "version": "10.9.4",
    "title": "Yogatik 10.9.4: Autonomous Relentless AI Engine — Markdown Table Loop-Breaker Fix & Pattern Precision",
    "date": "September 25, 2026",
    "isLatest": false,
    "highlights": [
      "Markdown Table False-Positive Fix: Stream repetition loop breaker no longer truncates valid markdown tables generated by NVIDIA Nemotron and other models",
      "Table-Aware Loop Detection: Pipe-delimited rows (| col | col |) and separator rows (| --- | --- |) are now exempt from both line-dedup and regex pattern matching",
      "Raised Pattern Minimum Length: Pattern length floor raised from 10→15 characters to prevent short table cell fragments triggering false loop detection",
      "Expanded Tail Window: Detection window enlarged from 300→500 chars giving richer context before any loop decision fires"
    ],
    "sections": [
      {
        ...
        "items": [
          {
            "title": "Markdown Table Exemption in Loop Breaker",
            "description": "Both the line-based deduplication (deduplicateRepetitions) and the stream-level regex pattern matcher (createReasoningTagger) now detect pipe-delimited markdown rows and skip loop detection entirely, preventing valid comparative tables from being cut off with '…repetitive text loop truncated'."
          },
          {
            "title": "Precision Pattern Threshold (10→15 chars)",
            "description": "The minimum pattern length for triggering the regex-based repetition detector was raised from 10 to 15 characters. This prevents short table cell tokens like ' --- ' or ' col ' from falsely matching as repeating units."
          },
          {
            "title": "Wider Detection Tail (300→500 chars)",
            "description": "The sliding tail window used by createReasoningTagger for both reasoning and content channels was extended from 300 to 500 characters, giving the detector richer context and making it more accurate at distinguishing real infinite loops from structured repeated content."
          }
        ]
      }
    ]
