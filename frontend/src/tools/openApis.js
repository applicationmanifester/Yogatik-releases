  ...
  async execute({ request = '', category = '' } = {}) {
    try {
      ...
      const keywordMappings = {
        ...
        learn: ['web_search', 'deep_research', 'wikipedia'],
        quantum: ['web_search', 'deep_research', 'wikipedia'],
        physics: ['web_search', 'deep_research', 'wikipedia'],
        code: ['code_execute', 'js_execute', 'code_validate'],
        programming: ['code_execute', 'js_execute', 'code_validate', 'aider_copilot'],
        debug: ['code_execute', 'js_execute', 'code_validate'],
        javascript: ['code_execute', 'js_execute', 'code_validate'],
        function: ['code_execute', 'js_execute', 'code_validate'],
        ...
        analyze: ['data_convert'],
        analysis: ['data_convert'],
        translate: ['translate'],
        ...
        calc: ['calculator'],
        search: ['web_search', 'deep_research', 'fs_find_files', 'fs_search'],
        ...
        system: ['system_state'],
        crypto: ['crypto_price'],
        stock: ['stock_analysis', 'zerodha_trade'],
        news: ['news_api'],
        podcast: ['podcast_gen']
      }

      // Apply keyword mappings using whole-token matching
      const wordTokens = new Set(text.match(/[a-z0-9_]+/g) || [])
      Object.entries(keywordMappings).forEach(([keyword, relatedTools]) => {
        const matches = keyword.includes(' ') ? text.includes(keyword) : wordTokens.has(keyword)
        if (matches) {
      ...
      // Get tool details for suggestions. tools/index.js statically imports THIS
      // file, so a static import back from here would be a circular import —
      // resolve the registry lazily instead (execute is async, so by call time
      // the registry is fully evaluated either way).
      let registry = {}
      try {
        const mod = await import('./index')
        registry = mod.ALL_TOOLS || mod.default || {}
      } catch { /* stays empty */ }
      const toolDetails = suggestions
        .map(toolName => {
          ...
          return {
            name: toolName,
            description: tool?.schema?.function?.description || tool?.schema?.description || `${toolName.replace(/_/g, ' ')} tool`,
