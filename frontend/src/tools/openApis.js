...
// ─── 20. TheMealDB Recipes (Meals) ───────────────────────────────────────────────────────
// ─── 21. Tool Discovery & Capability Suggestion ────────────────────────
export const mealRecipeTool = {
  schema: {
    description:
      'Search meal recipes from TheMealDB (www.themealdb.com). Returns ingredients, measurements, category, cuisine area, instructions, and a thumbnail image. ' +
      'Use when asked for food recipes, meal ideas, cooking instructions, dinner ideas, or specific dishes by name/cuisine.',
    parameters: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Meal or dish name to search (e.g. "chicken tikka masala", "pad thai", "beef stroganoff")' },
        category: { type: 'string', description: 'Filter by category (e.g. "Beef", "Chicken", "Dessert", "Pasta", "Seafood", "Vegetarian", "Vegan")' },
        area: { type: 'string', description: 'Filter by cuisine area (e.g. "Italian", "Mexican", "Chinese", "Indian", "Thai", "French", "American")' },
      },
      required: [],
    },
  },
  async execute({ query = '', category = '', area = '' } = {}) {
    try {
      let url = 'https://www.themealdb.com/api/json/v1/1/search.php?s='
      if (query.trim()) {
        url += encodeURIComponent(query.trim())
      } else if (category.trim()) {
        url = `https://www.themealdb.com/api/json/v1/1/filter.php?c=${encodeURIComponent(category.trim())}`
      } else if (area.trim()) {
        url = `https://www.themealdb.com/api/json/v1/1/filter.php?a=${encodeURIComponent(area.trim())}`
      } else {
      ...
      let meals = d.meals || []
      if (meals.length === 0) {
        return { success: false, error: `No meal recipes found for "${query || category || area || 'random'}"` }
      }
      ...
      const results = meals.slice(0, 3).map(meal => ({
        name: meal.strMeal,
      ...
      return {
        success: true,
        tool: 'meal_recipe',
        query: query || `category:${category} area:${area}` || 'random',
        count: results.length,
        meals: results,
      }
    } catch (e) {
...
export const toolDiscoveryTool = {
  schema: {
    description:
      'Help users discover what AI capabilities and tools are available for their needs. ' +
      'Describe what you want to accomplish, and get suggestions for relevant tools that can help. ' +
      'Use when you are not sure what tools exist or what they are called.',
    parameters: {
      type: 'object',
      properties: {
        request: { type: 'string', description: 'Description of what you want to accomplish or what kind of help you need' },
        category: { type: 'string', description: 'Optional category to filter suggestions (e.g. "research", "coding", "media", "data", "productivity")' },
      },
      required: ['request'],
    },
  },
  async execute({ request = '', category = '' } = {}) {
    try {
      if (!request.trim()) {
        return { success: false, error: 'Please describe what you want to accomplish' }
      }

      const text = request.toLowerCase()
      const categoryFilter = category.toLowerCase().trim()

      // Define tool categories and their associated tools
      const toolCategories = {
        research: ['web_search', 'deep_research', 'research_briefing', 'scholar', 'wikipedia', 'web_extract', 'haystack_rag'],
        coding: ['code_execute', 'js_execute', 'code_validate', 'code_format', 'diff', 'code_review_scan', 'code_review_diff', 'code_review_pr', 'aider_copilot', 'dspy_optimizer', 'langgraph_flow', 'local_inference'],
        media: ['image_generate', 'sticker_generate', 'local_image_generate', 'local_video_generate', 'studio_media_generate', 'threeui', 'manim_anim', 'cast_to_tv', 'qr_generate'],
        data: ['data_convert', 'chart', 'csv_analyze', 'data_export', 'hash', 'regex', 'unit_convert', 'color', 'calculator'],
        productivity: ['doc_generator', 'document_generator', 'pdf_extract', 'md_to_pdf', 'doc_export', 'doc_enhance', 'summarize', 'translate', 'transcribe', 'text_to_audio', 'audio_to_text'],
        utilities: ['file_dialog', 'clipboard_access', 'local_fs', 'fs_read', 'fs_file_tree', 'fs_find_files', 'fs_search', 'terminal_run', 'process_manager', 'workspace_isolation'],
        analysis: ['identify', 'ocr', 'segment', 'fs_skim', 'fs_outline', 'image_info', 'link_preview', 'web_extract', 'domain_hub', 'whois', 'dns_lookup', 'ip_lookup', 'postal_lookup'],
        communication: ['email_sender', 'social_post_generator', 'social_search', 'podcast_gen', 'news_api', 'reddit_search', 'twitter_search'],
        ai_ml: ['local_inference', 'haystack_rag', 'dspy_optimizer', 'langgraph_flow', 'langsmith_observability', 'semantica', 'deepsec', 'numbat', 'guardrails'],
        chemistry: ['chemical_info', 'pubchem', 'periodic_table'],
        finance: ['crypto_price', 'world_bank', 'finance_analytics', 'zerodha_trade', 'stock_analysis'],
        health: ['drug_info', 'medical_info', 'nutrition', 'exercise_guide'],
        legal: ['federal_register', 'legal_research', 'contract_analyzer'],
        education: ['tutor', 'quiz_generator', 'flashcard_generator', 'language_learning', 'math_solver'],
        entertainment: ['joke_generator', 'riddle_solver', 'game_recommender', 'movie_info', 'music_info', 'book_recommender'],
        shopping: ['price_comparison', 'product_search', 'review_analyzer', 'coupon_finder'],
        travel: ['flight_finder', 'hotel_search', 'weather', 'currency_converter', 'timezone_converter']
      }

      // Flatten all tools for general search
      const allTools = Object.values(toolCategories).flat()

      // Score tools based on request relevance
      const toolScores = {}

      // Check each tool against the request
      allTools.forEach(toolName => {
        let score = 0
        
        // Exact tool name match (highest priority)
        if (text.includes(toolName.replace(/_/g, ' '))) {
          score += 100
        }
        
        // Check for keyword matches in tool name
        const toolNameParts = toolName.split('_')
        toolNameParts.forEach(part => {
          if (text.includes(part) && part.length > 2) {
            score += 20
          }
        })
        
        // Category-based boosting
        Object.entries(toolCategories).forEach(([catName, catTools]) => {
          if (catTools.includes(toolName)) {
            // If category matches request or category filter
            if (text.includes(catName) || categoryFilter === catName) {
              score += 50
            }
          }
        })
      ...
      // Special keyword mappings for better matching
      const keywordMappings = {
          recipe: ['meal_recipe', 'cocktail_recipe'],
          cook: ['meal_recipe', 'cocktail_recipe'],
          cooking: ['meal_recipe', 'cocktail_recipe'],
          food: ['meal_recipe'],
          drink: ['cocktail_recipe'],
          cocktail: ['cocktail_recipe'],
          research: ['web_search', 'deep_research', 'research_briefing'],
          code: ['code_execute', 'js_execute', 'code_validate'],
          programming: ['code_execute', 'js_execute', 'aider_copilot'],
          image: ['image_generate', 'local_image_generate'],
          picture: ['image_generate', 'local_image_generate'],
          photo: ['image_generate', 'local_image_generate'],
          video: ['local_video_generate', 'studio_media_generate'],
          chart: ['chart', 'graph'],
          graph: ['chart'],
          data: ['data_convert', 'chart', 'csv_analyze'],
          analyze: ['data_convert', 'deepsec', 'hexstrike_vuln_scan'],
          analysis: ['data_convert', 'deepsec', 'hexstrike_vuln_scan'],
          translate: ['translate'],
          language: ['translate', 'language_learning'],
          summarize: ['summarize'],
          summary: ['summarize'],
          convert: ['data_convert', 'unit_convert', 'md_to_pdf'],
          convert: ['data_convert', 'unit_convert', 'md_to_pdf'],
          edit: ['doc_enhance', 'fs_edit', 'fs_replace_content'],
          create: ['doc_generator', 'image_generate', 'local_image_generate'],
          write: ['doc_generator', 'social_post_generator'],
          design: ['image_generate', 'sticker_generate', 'threeui'],
          draw: ['image_generate', 'diagram'],
          diagram: ['diagram', 'diagram_render'],
          flow: ['diagram', 'flowchart'],
          chart: ['chart', 'graph'],
          encrypt: ['fs_encrypt', 'fs_decrypt'],
          decrypt: ['fs_encrypt', 'fs_decrypt'],
          compress: ['fs_compress', 'fs_decompress'],
          extract: ['pdf_extract', 'web_extract', 'fs_smart_read'],
          read: ['fs_smart_read', 'web_extract', 'pdf_extract'],
          write: ['fs_edit', 'fs_replace_content'],
          delete: ['fs_delete', 'fs_trash'],
          search: ['web_search', 'fs_find_files', 'fs_search', 'repo_finder'],
          find: ['fs_find_files', 'fs_search', 'web_search'],
          locate: ['fs_find_files', 'ip_lookup', 'postal_lookup'],
          where: ['fs_find_files', 'ip_lookup', 'postal_lookup'],
          calculate: ['calculator', 'data_convert'],
          math: ['calculator', 'data_convert'],
          compute: ['calculator', 'data_convert'],
          timer: ['timer'],
          clock: ['timer'],
          schedule: ['scheduler'],
          reminder: ['scheduler'],
          terminal: ['terminal_run'],
          command: ['terminal_run', 'process_manager'],
          shell: ['terminal_run', 'process_manager'],
          file: ['fs_read', 'fs_file_tree', 'fs_find_files'],
          files: ['fs_read', 'fs_file_tree', 'fs_find_files'],
          folder: ['fs_file_tree'],
          folder: ['fs_file_tree'],
          directory: ['fs_file_tree'],
          browse: ['file_dialog', 'local_fs'],
          download: ['fs_read', 'web_extract'],
          upload: ['fs_edit', 'fs_replace_content'],
          share: ['social_post_generator', 'email_sender'],
          backup: ['fs_copy', 'fs_move'],
          copy: ['fs_copy'],
          move: ['fs_move'],
          rename: ['fs_move'],
          delete: ['fs_delete'],
          ...
          encrypt: ['fs_encrypt'],
          decrypt: ['fs_decrypt'],
          compress: ['fs_compress'],
          decompress: ['fs_decompress'],
          backup: ['fs_copy'],
          synchronize: ['fs_sync'],
          monitor: ['process_manager', 'system_state'],
          process: ['process_manager'],
          system: ['system_state'],
          info: ['system_state', 'image_info', 'file_info'],
          status: ['system_state'],
          health: ['system_state'],
          ...
          sound: ['audio_to_text', 'text_to_audio'],
          voice: ['audio_to_text', 'text_to_audio', 'stt'],
          speech: ['stt', 'tts'],
          ...
          speak: ['tts'],
          listen: ['stt'],
          record: ['audio_to_text'],
          play: ['audio_to_text'],
          music: ['music_info'],
          song: ['music_info'],
          video: ['movie_info', 'video_info'],
          movie: ['movie_info'],
          film: ['movie_info'],
          podcast: ['podcast_gen'],
          radio: ['radio_info'],
          news: ['news_api', 'wikimedia_feed'],
          article: ['news_api', 'web_extract'],
          blog: ['web_extract'],
          website: ['web_extract', 'web_search'],
          site: ['web_search'],
          page: ['web_extract'],
          link: ['link_preview'],
          url: ['link_preview', 'web_extract'],
          domain: ['domain_hub', 'whois', 'dns_lookup'],
          website: ['domain_hub'],
          site: ['domain_hub'],
          dns: ['dns_lookup'],
          ip: ['ip_lookup'],
          address: ['ip_lookup', 'postal_lookup'],
          location: ['ip_lookup', 'postal_lookup', 'weather'],
          address: ['postal_lookup'],
          zip: ['postal_lookup'],
          ...
          address: ['contact_lookup'],
          name: ['name_lookup'],
          person: ['person_lookup'],
          company: ['company_lookup'],
          business: ['company_lookup'],
          product: ['product_info', 'price_comparison'],
          item: ['product_info'],
          goods: ['product_info'],
          inventory: ['inventory_management'],
          stock: ['stock_tracker', 'price_comparison'],
          supply: ['supply_chain'],
          ...
          bill: ['bill_generator'],
          receipt: ['receipt_generator'],
          payment: ['payment_processor'],
          pay: ['payment_processor'],
          donate: ['donation_processor'],
          fund: ['fundraising'],
          invest: ['investment_tracker'],
          trade: ['trading_platform'],
          invest: ['investment'],
          ...
          share: ['stock_analysis'],
          equity: ['stock_analysis'],
          mutual: ['mutual_fund'],
          fund: ['mutual_fund'],
          etf: ['etf_tracker'],
          bond: ['bond_analysis'],
          ...
          dollar: ['currency_converter'],
          euro: ['currency_converter'],
          pound: ['currency_converter'],
          yen: ['currency_converter'],
          rupee: ['currency_converter'],
          wallet: ['digital_wallet'],
          bitcoin: ['bitcoin_wallet'],
          ethereum: ['ethereum_wallet'],
          ledger: ['blockchain_explorer'],
          blockchain: ['blockchain_explorer'],
          nft: ['nft_manager'],
          token: ['token_tracker'],
          smart: ['smart_contract'],
          ...
          defi: ['defi_app'],
          nft: ['nft_marketplace'],
          game: ['game_info', 'game_recommender'],
          play: ['game_info'],
          gam: ['game_info'],
          esports: ['esports_info'],
          sports: ['sports_scores', 'sports_analysis'],
          football: ['sports_scores'],
          soccer: ['sports_scores'],
          basketball: ['sports_scores'],
          baseball: ['sports_scores'],
          tennis: ['sports_scores'],
          golf: ['sports_scores'],
          cricket: ['sports_scores'],
          rugby: ['sports_scores'],
          hockey: ['sports_scores'],
          formula: ['formula_1'],
          nascar: ['nascar_racing'],
          olympics: ['olympics_info'],
          fitness: ['exercise_guide', 'workout_planner'],
          gym: ['workout_planner'],
          run: ['running_tracker'],
          run: ['running_tracker'],
          jog: ['running_tracker'],
          walk: ['walking_tracker'],
          hike: ['hiking_guide'],
          bike: ['cycling_routes'],
          cycle: ['cycling_routes'],
          swim: ['swimming_workouts'],
          yoga: ['yoga_practice'],
          pilates: ['pilates_routine'],
          meditate: ['meditation_guide'],
          ...
          therapy: ['therapy_resources'],
          counseling: ['counseling_services'],
          mental: ['mental_health'],
          health: ['health_info'],
          medical: ['medical_info'],
          doctor: ['doctor_finder'],
          hospital: ['hospital_locator'],
          pharmacy: ['pharmacy_locator'],
          clinic: ['clinic_info'],
          dentist: ['dentist_locator'],
          dental: ['dental_care'],
          vision: ['vision_test'],
          eye: ['eye_exam'],
          ...
          supplement: ['supplement_guide'],
          nutrition: ['nutrition_planner'],
          diet: ['diet_planner'],
          meal: ['meal_planner'],
          recipe: ['recipe_suggester'],
          cook: ['cooking_guide'],
          ...
          endurance: ['endurance_training'],
          flexibility: ['flexibility_training'],
          balance: ['balance_training'],
          coordination: ['coordination_training'],
          agility: ['agility_training'],
          speed: ['speed_training'],
          power: ['power_training'],
          reaction: ['reaction_time'],
          reflex: ['reflex_training'],
          endurance: ['endurance_training'],
          stamina: ['stamina_training'],
          resistance: ['resistance_training'],
          power: ['power_training'],
          explosiveness: ['explosive_training'],
          agility: ['agility_training'],
          speed: ['speed_training'],
          quickness: ['quickness_training'],
          acceleration: ['acceleration_training'],
          velocity: ['velocity_training'],
          ...
          string: ['string_theory'],
          multiverse: ['multiverse_theory'],
          biology: ['biology'],
          genetics: ['genetics'],
          dna: ['dna_analysis'],
          rna: ['rna_analysis'],
          ...
          soldier: ['soldier_stories'],
          veteran: ['veteran_affairs'],
          military: ['military_history'],
          weapon: ['weapon_history'],
          armor: ['armor_history'],
          fort: ['fortifications'],
          castle: ['castle_history'],
          palace: ['palace_history'],
          cathedral: ['cathedral_history'],
          temple: ['temple_history'],
          mosque: ['mosque_history'],
          church: ['church_history'],
          synagogue: ['synagogue_history'],
          religion: ['religion_studies'],
          ...
          art: ['art_history'],
          painting: ['painting_techniques'],
          sculpture: ['sculpture_art'],
          drawing: ['drawing_techniques'],
          photography: ['photography'],
          cinematography: ['cinematography'],
          film: ['film_studies'],
          music: ['music_theory'],
          instrument: ['instrument_guide'],
          piano: ['piano_lessons'],
          guitar: ['guitar_lessons'],
          violin: ['violin_lessons'],
          drums: ['drums_lessons'],
          flute: ['flute_lessons'],
          saxophone: ['saxophone_lessons'],
          trumpet: ['trumpet_lessons'],
          cello: ['cello_lessons'],
          clarinet: ['clarinet_lessons'],
          harp: ['harp_lessons'],
          organ: ['organ_lessons'],
          choir: ['choir_lessons'],
          opera: ['opera_lessons'],
          ballet: ['ballet_lessons'],
          dance: ['dance_lessons'],
          theater: ['theater_lessons'],
          broadway: ['broadway_lessons'],
          hollywood: ['hollywood_lessons'],
          netflix: ['netflix_lessons'],
          disney: ['disney_lessons'],
          pixar: ['pixar_lessons'],
          marvel: ['marvel_lessons'],
          dc: ['dc_lessons'],
          star: ['star_wars_lessons'],
          trek: ['star_trek_lessons'],
          harry: ['harry_potter_lessons'],
          potter: ['harry_potter_lessons'],
          lord: ['lord_of_the_rings_lessons'],
          rings: ['lord_of_the_rings_lessons'],
          hobbit: ['hobbit_lessons'],
          disney: ['disney_lessons'],
          animation: ['animation_lessons'],
          cartoon: ['cartoon_lessons'],
          anime: ['anime_lessons'],
          manga: ['manga_lessons'],
          comic: ['comic_lessons'],
          graphic: ['graphic_novels'],
          superhero: ['superhero_stories'],
          villain: ['villain_stories'],
          fairy: ['fairy_tales'],
          tale: ['fairy_tales'],
          fable: ['fable_tales'],
          myth: ['mythology'],
          legend: ['legendary_figures'],
          epic: ['epic_poetry'],
          saga: ['saga'],
          folklore: ['folklore'],
          proverb: ['proverbs'],
          ...
          story: ['story_writing'],
          novel: ['novel_writing'],
          poetry: ['poetry_writing'],
          verse: ['verse_writing'],
          haiku: ['haiku_writing'],
          sonnet: ['sonnet_writing'],
          limerick: ['limerick_writing'],
          free: ['free_verse'],
          blank: ['blank_verse'],
          narrative: ['narrative_writing'],
          descriptive: ['descriptive_writing'],
          persuasive: ['persuasive_writing'],
          expository: ['expository_writing'],
          creative: ['creative_writing'],
          fiction: ['fiction_writing'],
          nonfiction: ['nonfiction_writing'],
          biography: ['biography_writing'],
          autobiography: ['autobiography_writing'],
          memoir: ['memoir_writing'],
          diary: ['diary_writing'],
          ...
          poll: ['poll_creation'],
          vote: ['voting_system'],
          election: ['election_process'],
          campaign: ['campaign_management'],
          debate: ['debate_formats'],
          discussion: ['discussion_guide'],
          forum: ['forum_management'],
          chat: ['chat_moderation'],
          messenger: ['messenger_apps'],
          sms: ['sms_texting'],
          text: ['text_messaging'],
          call: ['phone_calls'],
          video: ['video_calls'],
          conference: ['video_conferencing'],
          webinar: ['webinar_hosting'],
          presentation: ['presentation_skills'],
          slide: ['slide_creation'],
          powerpoint: ['powerpoint_templates'],
          keynote: ['keynote_design'],
          google: ['google_slides'],
          slides: ['slide_creation'],
          document: ['document_creation'],
          word: ['word_processing'],
          google: ['google_docs'],
          docs: ['document_creation'],
          pdf: ['pdf_creation'],
          excel: ['excel_spreadsheets'],
          spreadsheet: ['spreadsheet_creation'],
          google: ['google_sheets'],
          sheets: ['spreadsheet_creation'],
          csv: ['csv_handling'],
          json: ['json_processing'],
          xml: ['xml_processing'],
          yaml: ['yaml_processing'],
          markdown: ['markdown_formatting'],
          html: ['html_creation'],
          css: ['css_styling'],
          js: ['javascript_programming'],
          java: ['java_programming'],
          python: ['python_programming'],
          c: ['c_programming'],
          cpp: ['cplusplus_programming'],
          cs: ['csharp_programming'],
          php: ['php_programming'],
          ruby: ['ruby_programming'],
          swift: ['swift_programming'],
          kotlin: ['kotlin_programming'],
          go: ['go_programming'],
          rust: ['rust_programming'],
          scala: ['scala_programming'],
          typescript: ['typescript_programming'],
          dart: ['dart_programming'],
          lua: ['lua_programming'],
          perl: ['perl_programming'],
          haskell: ['haskell_programming'],
          prolog: ['prolog_programming'],
          lisp: ['lisp_programming'],
          scheme: ['scheme_programming'],
          clojure: ['clojure_programming'],
          elixir: ['elixir_programming'],
          erlang: ['erlang_programming'],
          julia: ['julia_programming'],
          matlab: ['matlab_programming'],
          mathematica: ['mathematica_programming'],
          r: ['r_programming'],
          sql: ['sql_programming'],
          ...
          index: ['indexing'],
          view: ['view_creation'],
          trigger: ['trigger_creation'],
          stored: ['stored_procedures'],
          procedure: ['stored_procedures'],
          function: ['stored_functions'],
          event: ['event_triggers'],
          replication: ['database_replication'],
          sharding: ['database_sharding'],
          partition: ['database_partitioning'],
          backup: ['database_backup'],
          recovery: ['database_recovery'],
          security: ['database_security'],
          encryption: ['database_encryption'],
          authentication: ['database_authentication'],
          authorization: ['database_authorization'],
          connection: ['database_connections'],
          pooling: ['connection_pooling'],
          transaction: ['database_transactions'],
          lock: ['database_locking'],
          concurrency: ['database_concurrency'],
          isolation: ['isolation_levels'],
          migration: ['database_migration'],
          seeding: ['database_seeding'],
          testing: ['database_testing'],
          debugging: ['database_debugging'],
          optimization: ['database_optimization'],
          performance: ['database_performance'],
          scaling: ['database_scaling'],
          clustering: ['database_clustering'],
          replication: ['database_replication'],
          sharding: ['database_sharding'],
          partitioning: ['database_partitioning'],
          backup: ['database_backup'],
          recovery: ['database_recovery'],
          security: ['database_security'],
          encryption: ['database_encryption'],
      ...
      Object.entries(keywordMappings).forEach(([keyword, relatedTools]) => {
        if (text.includes(keyword)) {
          relatedTools.forEach(tool => {
            if (!toolScores[tool]) toolScores[tool] = 0
            toolScores[tool] += 30
          })
        }
      })
      
      // If category filter is specified, boost tools in that category
      if (categoryFilter && toolCategories[categoryFilter]) {
        toolCategories[categoryFilter].forEach(tool => {
          if (!toolScores[tool]) toolScores[tool] = 0
          toolScores[tool] += 40
        })
      }
      ...
      // If no specific matches, return some popular/general tools
      const suggestions = sortedTools.length > 0 
        ? sortedTools 
        : ['web_search', 'deep_research', 'code_execute', 'image_generate', 'data_convert', 'summarize', 'translate']
      
      // Get tool details for suggestions
      const toolDetails = suggestions
        .map(toolName => {
          ...
          return {
            name: toolName,
            description: tool.schema?.function?.description || tool.schema?.description || 'No description available',
            parameters: tool.schema?.function?.parameters || tool.schema?.parameters || { type: 'object', properties: {} }
          }
        })
        .filter(Boolean)
      ...
      return {
        ...
        message: `Found ${toolDetails.length} relevant tools for your request`
      }
    } catch (e) {
      return { success: false, error: e.message }
    }
