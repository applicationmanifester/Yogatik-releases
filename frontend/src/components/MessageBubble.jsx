      ...
      <div className="message-role">
        ...
        {msg.role === 'assistant' && (msg.model || msg.provider) && (
          <span className="msg-model-badge" title={`Generated by ${msg.provider ? `${msg.provider} (${msg.model || 'default'})` : msg.model}`}>
            {msg.model ? (msg.provider ? `${msg.provider} / ${msg.model.split('/').pop()}` : msg.model.split('/').pop()) : (msg.provider === 'local' ? 'local' : msg.provider)}
          </span>
        )}
          ...
          <ol>
            {msg.trace?.length > 0 ? (
              ...
              <li className="trace-step trace-done">
                <Wrench size={11} /> <span className="trace-tool">Step 1: Direct response generation ({msg.model ? (msg.provider ? `${msg.provider} · ${msg.model.split('/').pop()}` : msg.model.split('/').pop()) : (msg.provider === 'local' ? 'On-device Model' : msg.provider || 'AI Model')})</span>
