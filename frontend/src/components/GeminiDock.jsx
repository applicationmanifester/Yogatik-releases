/**
 * GeminiDock — Side-panel dock for Google Gemini integration.
 * Provides a floating panel to interact with Gemini models alongside the main chat.
 */
import React, { useState, useEffect, useRef } from 'react'
import { X, Sparkles, Send, RefreshCw } from 'lucide-react'

export function GeminiDock({ onClose }) {
  ...
  async function handleSend() {
    ...
    try {
      ...
      if (!apiKey) {
        setMessages(prev => [...prev, { role: 'assistant', text: 'No Gemini API key set. Add it under Settings → Providers → Gemini.' }])
        return
      }
  ...
  return (
    <div className="gemini-dock" style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 16px', borderBottom: '1px solid var(--border, #2a2a3e)' }}>
        ...
        <span style={{ fontWeight: 600, fontSize: 14 }}>Gemini Dock</span>
        <span style={{ fontSize: 11, opacity: 0.5, marginLeft: 'auto' }}>Google</span>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        {messages.length === 0 && (
          <div style={{ opacity: 0.4, fontSize: 13, textAlign: 'center', marginTop: 32 }}>
            Ask Gemini anything alongside your main conversation.
          </div>
        )}
        {messages.map((m, i) => (
          <div key={i} style={{
            ...
            background: m.role === 'user' ? '#1a73e822' : 'var(--bg-tertiary, #2a2a3e)',
      ...
      <div style={{ padding: '8px 12px', borderTop: '1px solid var(--border, #2a2a3e)', display: 'flex', gap: 8 }}>
        ...
        <button
          ...
          style={{
            background: '#1a73e8', border: 'none', borderRadius: 8, padding: '7px 12px',
